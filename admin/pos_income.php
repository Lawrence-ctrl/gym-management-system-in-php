<?php
  ...
  if(isset($_POST['submit'])) {
    $from = $_POST['from'];
    $to= $_POST['to'];
    $expend = $conn->query("SELECT pos_income.*,pos.*,users.* FROM pos_income JOIN pos ON pos_income.poss_id=pos.pos_id JOIN users ON pos_income.poss_user_id = users.userid  WHERE poss_status='1' AND pos_created_date BETWEEN '$from' AND '$to'");
  }elseif(isset($_GET['id'])){
    $id = clean($_GET['id']);
    $expend = $conn->query("SELECT pos_income.*,pos.*,users.* FROM pos_income JOIN pos ON pos_income.poss_id=pos.pos_id JOIN users ON pos_income.poss_user_id = users.userid  WHERE poss_status='1' AND posi_id = '$id'");
  }else{
   $expend = $conn->query("SELECT pos_income.*,pos.*,users.* FROM pos_income JOIN pos ON pos_income.poss_id=pos.pos_id JOIN users ON pos_income.poss_user_id = users.userid WHERE poss_status='1'");
  }
?>
<!DOCTYPE html>
<html lang="en">
          ...
          <?php include('includes/navbar.php'); ?>       
              <div class="container-fluid">         
                  <div class="d-sm-flex align-items-center justify-content-between mb-4">               
                        <h1 class="h3 mb-0 text-gray-800">POS Income</h1>   
                ...
                <div class="text-center">
                  <form action="pos_income.php" method="POST">
                      ...
                      <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
                       ...
                       <tbody>
                        <?php while($sele = $expend->fetch_assoc()): 
                            $total += $sele['poss_total'];
                          ?>
                        <tr>
                           <td><?php echo $sele['posi_id']?></td>
                           <td><?php echo $sele['poss_id']?></td>
                           <td><?php echo $sele['pos_name']?></td>
                           <td><?php echo $sele['username']?></td>
                           <td><?php echo $sele['poss_price']?></td>
                           <td><?php echo $sele['poss_quantity']?></td>
                           <td><?php echo $sele['poss_total']?></td>
                           <td><?php echo $sele['pos_created_date']?></td>
                       </tr>
                       <?php endwhile; ?>
                       </tbody>
                    </table>
