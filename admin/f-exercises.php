<?php
  ...
  {
    if($_GET['cat_id'] == '1') {
       $exe = $conn->query("SELECT * FROM exercises WHERE gender='2' and exer_plan='1'");
    }elseif($_GET['cat_id']) {
       $cat_id = mysqli_real_escape_string($conn,$_GET['cat_id']);
       $exe = $conn->query("SELECT * FROM exercises WHERE exer_category_id='$cat_id' and gender='2'"); 
    } 
  }else{
      $exe = $conn->query("SELECT * FROM exercises WHERE gender='2' and exer_plan='1'");
  }  
              ...
              <div class="container-fluid">         
                  <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <?php 
                     ...
                     <?php }else{ ?>
                        <h1 class="h3 mb-0 text-gray-800">Exercises (Female)</h1>
                     <?php } ?>                         
                  </div> 
                       ...
                       <?php $result = $conn->query("SELECT * FROM exercises_category");
                        while($row = $result->fetch_assoc()): ?>
                          <div class="col-md-2 col-sm-6 col-xs-6">
                             <a href="?cat_id=<?php echo $row['cat_id']?>" class="btn btn-outline-secondary btn-lg btn-block he"><?php echo $row['exercises_name']?>                               
                             </a>
                          </div>
                       <?php endwhile; ?>
                    </div>
